...
import { getDetailCache, setDetailCache as setDetCache, getAllCachedGames } from './gameCache.js';
...
function getCached(key) {
  try {
    ...
    if (Date.now() - t > CACHE_TTL) { localStorage.removeItem(`${CACHE_KEY}_${key}`); return null; }
    return d;
  } catch { return null; }
}

function setCache(key, data) {
  try { localStorage.setItem(`${CACHE_KEY}_${key}`, JSON.stringify({ d: data, t: Date.now() })); }
  catch { /* quota */ }
}
...
export function clearAllCaches() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (k?.startsWith('gh_') || k?.startsWith('gamehub_cache')) toRemove.push(k);
  }
...
export const SOURCES = [
  ...
  { slug: 'gamedistribution', label: 'GameDistribution',  icon: '🎯', key: 'gd' },
...
const _ALIAS_MAP = (() => {
  const m = {};
  const add = (slug, ...aliases) => aliases.forEach(a => { m[a.toLowerCase().trim()] = slug; });

  add('action',      'action');
  add('adventure',   'adventure');
  add('arcade',      'arcade', 'stunt');
  add('puzzle',      'puzzle', 'brain', 'match-3', 'match3', 'word', 'trivia', 'quiz', 'match 3', 'jigsaw', 'agility');
  add('racing',      'racing', 'driving', 'car', 'motorcycle', 'drift');
  add('sports',      'sports', 'sport', 'soccer', 'football', 'basketball', 'baseball', 'cricket', 'golf', 'tennis', 'boxing', 'wrestling');
  add('shooting',    'shooting', 'shooter', 'fps', 'gun', 'sniper');
  add('strategy',    'strategy', 'tower-defense', 'tower defense', 'defense', 'td');
  add('multiplayer', 'multiplayer', '2-player', '2 player', 'two player');
  add('io',          'io', '.io');
  add('hypercasual', 'hypercasual', 'hyper-casual', 'hyper casual', 'casual');
  add('simulation',  'simulation', 'simulator', 'sim', 'tycoon', 'management', 'cooking', 'farming', 'building');
  add('board',       'board', 'card', 'boardgame', 'board game', 'card game', 'tabletop', 'chess', 'solitaire', 'mahjong');
  add('clicker',     'clicker', 'idle', 'incremental', 'tap');
  add('action',      'battle');
  // HTMLGames uses verbose category names like "Mahjong Games", "Puzzle Games"
  add('puzzle',      'puzzle games', 'brain games', 'bubble shooter games', 'match 3 games', 'word games', 'memory games', 'number games', 'slide games', 'connect games', 'tetris games', 'sokoban games');
  add('board',       'mahjong games', 'solitaire games', 'card games', 'board games');
  add('arcade',      'arcade games', 'platform games', 'snake games', 'block games', 'breakout games');
...
function normalizeCategory(raw) {
  if (!raw) return 'other';
  const lower = (typeof raw === 'string' ? raw : Array.isArray(raw) ? raw[0] : String(raw))
    .toLowerCase().trim();
...
export function getCategoryMeta(slug) {
  return CATEGORIES.find(c => c.slug === slug) || { slug, label: slug, icon: '🎮' };
}

// ── Per-source category name for API query params ──
const _SRC_CAT = {
  gm: { action: 'Action', adventure: 'Adventure', arcade: 'Arcade', puzzle: 'Puzzle', racing: 'Racing', sports: 'Sports', shooting: 'Shooting', strategy: 'Strategy', multiplayer: 'Multiplayer', io: '.io', hypercasual: 'Hypercasual', simulation: 'Simulation', board: 'Board', clicker: 'Clicker' },
  gd: { action: 'Action', adventure: 'Adventure', arcade: 'Arcade', puzzle: 'Puzzle', racing: 'Racing', sports: 'Sports', shooting: 'Shooting', strategy: 'Strategy', multiplayer: 'Multiplayer', io: 'IO', hypercasual: 'Hypercasual', simulation: 'Simulation', board: 'Board', clicker: 'Clicker' },
  gp: { action: 'action', adventure: 'adventure', arcade: 'arcade', puzzle: 'puzzle', racing: 'racing', sports: 'sports', shooting: 'shooting', strategy: 'strategy', multiplayer: 'multiplayer', io: 'io', hypercasual: 'hypercasual', simulation: 'simulation', board: 'board', clicker: 'clicker' },
};
...
// In dev, Vite proxies /api/gm → gamemonetize.com (avoids CORS).
// In production, use a serverless proxy or Cloudflare Worker.
const GM_BASE = import.meta.env.DEV
  ? '/api/gm'
  : 'https://gamemonetize.com';

async function rawGM(category, page, amount) {
  const params = new URLSearchParams({ format: 'json', page: String(page), amount: String(amount) });
  if (category) params.set('category', category);
  const res = await fetch(`${GM_BASE}/rssfeed.php?${params}`, { signal: AbortSignal.timeout(TIMEOUT) });
...
async function rawGP(category, page, amount) {
  ...
  const params = new URLSearchParams({ page: String(page), num: String(amount), order: 'q' });
  if (category) params.set('category', category);

  // Try the public HTML5 games endpoint first
  try {
    const res = await fetch(`https://games.gamepix.com/games?${params}`, { signal: AbortSignal.timeout(TIMEOUT) });
    if (res.ok) {
      const json = await res.json();
  ...
  try {
    const res2 = await fetch(`https://feeds.gamepix.com/v2/json?${params}`, { signal: AbortSignal.timeout(TIMEOUT) });
    if (res2.ok) {
...
async function rawHG(category, page, amount) {
  ...
  if (!_hgCache || Date.now() - _hgCacheTime > HG_CACHE_TTL) {
    try {
      const hgBase = import.meta.env.DEV ? '/api/hg' : 'https://www.htmlgames.com';
...
function parseGD(g) {
  const thumb = Array.isArray(g.Asset) ? (g.Asset.find(u => u?.includes('512x384')) || g.Asset[0] || '') : '';
...
function parseHG(g) {
  ...
  return {
    ...
    description: str(g.description),
    thumbnail: str(g.thumb5 || g.thumb6 || g.thumb4 || g.thumb3 || g.thumb2 || g.thumb1),
    url: str(g.url),
    category: normalizeCategory(rawCat),
    categoryRaw: rawCat,
    tags: '',
...
async function fetchSourcePage(src, category, page, amount) {
  switch (src) {
    case 'gm': return (await rawGM(sourceCat(category, 'gm'), page, amount)).map(parseGM);
    case 'gd': return (await rawGD(sourceCat(category, 'gd'), page, amount)).map(parseGD);
    case 'gp': return (await rawGP(sourceCat(category, 'gp'), page, amount)).map(parseGP);
    case 'hg': return (await rawHG(category, page, amount)).map(parseHG);
    default: return [];
  }
}
...
export function initCursors() {
  return { gm: 1, gd: 1, gp: 1, hg: 1, gmDone: false, gdDone: false, gpDone: false, hgDone: false };
}
...
export async function fetchGamesBatch({ category = 'all', cursors, amount = PER_SOURCE, sourceFilter = null } = {}) {
  ...
  const activeSrcs = sourceFilter
    ? (enabledSrcs.includes(sourceFilter) ? [sourceFilter] : [sourceFilter]) // still allow single-source fetch even if disabled
 ...
 */
export async function fetchGames({ category = 'all', page = 1, limit = 40 } = {}) {
  const cacheKey = `f_${category}_${page}_${limit}`;
  const cached = getCached(cacheKey);
  if (cached) return cached;

  const cursors = { gm: page, gd: page, gp: page, hg: page, gmDone: false, gdDone: false, gpDone: false, hgDone: false };
  ...
  if (result.length === 0) throw new Error('All game sources unavailable. Check your connection.');
  return result;
}
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    try {
      ...
      if (Array.isArray(d)) {
        ...
        if (found) { setCache(ck, found); setDetCache(id, found); return found; }
      }
    } catch { /* skip */ }
  }
  ...
  if (pool.length < 200) {
    try {
      ...
      for (const g of fresh) {
        if (!ids.has(g.id)) { pool.push(g); ids.add(g.id); }
      }
