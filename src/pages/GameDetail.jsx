...
import { Play, Heart, Share2, ArrowLeft, Clock, Tag, User, Building2, Monitor, Calendar, Loader2, Download, CheckCircle, ThumbsUp, ThumbsDown, FolderPlus, Plus, X } from 'lucide-react';
...
export default function GameDetail() {
  ...
  const { collections, createCollection, addGameToCollection, isGameInCollection } = useCollections();
  ...
  const handleShare = async () => {
    if (!game) return;
    try {
      await navigator.share({ title: game.title, text: `Play ${game.title} on GameHub!`, url: window.location.href });
    } catch {
      ...
      <div className="min-h-screen bg-dark-900 max-w-lg mx-auto">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-2 mt-6">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="skeleton-shimmer h-3 rounded-md" style={{ width: `${90 - i * 15}%` }} />
            ))}
          </div>
        </div>
      </div>
  ...
  if (!game) {
    return (
      <div className="min-h-screen bg-dark-900 max-w-lg mx-auto flex flex-col items-center justify-center p-8">
        ...
        <h2 className="font-display text-lg font-bold text-white mb-2">Game Not Found</h2>
        <p className="text-dim text-sm mb-6">This game may have been removed</p>
        <button onClick={() => navigate('/')} className="btn-cyber text-sm">Go Home</button>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-dark-900 max-w-lg mx-auto">
        ...
        <div className="relative h-64 overflow-hidden">
          {game.thumbnail && !imgFailed ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-dark-600 to-dark-800 flex items-center justify-center text-6xl">
              {icon}
            </div>
          )}
          ...
          <div className="absolute top-0 left-0 right-0 flex items-center justify-between p-3">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => saved ? removeOffline(game.id) : saveOffline(game)}
        ...
        <div className="relative -mt-8 rounded-t-3xl bg-dark-900 px-5 pt-6 pb-8">
          ...
          <div className="flex items-center gap-2 mb-3">
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-neon-cyan/10 border border-neon-cyan/20 text-neon-cyan text-[10px] font-display font-bold tracking-wider uppercase">
              {icon} {getCategoryMeta(game.category)?.label || game.categoryRaw || 'Game'}
            </span>
          ...
          <p className="text-[11px] text-dim mb-5">
            via {game.source === 'gamemonetize' ? 'GameMonetize' : game.source === 'gamedistribution' ? 'GameDistribution' : game.source === 'htmlgames' ? 'HTMLGames' : 'GamePix'}
          </p>
          ...
          >
            {[
              { icon: <Tag size={14} />, label: 'Category', value: getCategoryMeta(game.category)?.label || game.categoryRaw || 'N/A' },
              { icon: <Monitor size={14} />, label: 'Platform', value: 'HTML5 / Mobile' },
              { icon: <Building2 size={14} />, label: 'Source', value: game.source || 'Web' },
              { icon: <Clock size={14} />, label: 'Type', value: 'Free to Play' },
            ].map(info => (
              <div key={info.label} className="p-3 rounded-xl bg-dark-700/50 border border-dark-500/20">
                <div className="flex items-center gap-1.5 text-neon-cyan/60 mb-1">
                  {info.icon}
                  <span className="text-[10px] font-display font-bold tracking-wider uppercase">{info.label}</span>
                </div>
          ...
          {game.tags && (
            ...
            >
              ...
              <div className="flex flex-wrap gap-1.5">
                {game.tags.split(',').map(tag => tag.trim()).filter(Boolean).slice(0, 8).map(tag => (
                  <span key={tag} className="px-2.5 py-1 rounded-lg bg-dark-600/50 border border-dark-400/20 text-[11px] text-dim">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-display text-sm font-bold text-white tracking-wider">ADD TO COLLECTION</h3>
                <button onClick={() => setShowColPicker(false)} className="text-dim hover:text-white">
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="space-y-2 mb-4 max-h-48 overflow-y-auto">
                {collections.length === 0 && (
                  <p className="text-xs text-dim text-center py-4">No collections yet — create one below!</p>
                )}
                {collections.map(col => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[10px] text-dim">{col.games.length} games</span>
                ...
                <input
                  ...
                  onKeyDown={e => {
                    if (e.key === 'Enter' && newColName.trim()) {
                      ...
                      toast(`Created "${newColName.trim()}" and added game`, 'success');
                      setNewColName('');
                    }
                  }}
                />
